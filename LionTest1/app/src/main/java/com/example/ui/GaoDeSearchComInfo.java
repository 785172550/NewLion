 ...
 */
public class GaoDeSearchComInfo  extends BaseActivity implements
	...
	private void init() {
		...
		initTopBarForBothWithText("选择公司地址", "确定", new HeaderLayout.onRightImageButtonClickListener() {
			@Override
			public void onClick(View v) {
				// 判断是否选择了地点
				if (selected_position_lat.isEmpty() || selected_positios_long.isEmpty()) {
					ShowToast("请搜索并选择您的公司地址");
					return;
				}

				// 返回信息
				Intent intent = getIntent();
				intent.putExtra("selected_position_lat", selected_position_lat);
				intent.putExtra("selected_positios_long", selected_positios_long);
				setResult(RESULT_OK, intent);
				GaoDeSearchComInfo.this.finish();
			}
	...
	protected void doSearchQuery() {
		...
		selected_positios_long = "";
		selected_position_lat = "";
	}
	...
	public boolean onMarkerClick(Marker marker) {
		...
		// 获取经纬度
		selected_position_lat = marker.getPosition().latitude + "";
		selected_positios_long = marker.getPosition().longitude + "";
		return false;
	}
	...
	public void onTextChanged(CharSequence s, int start, int before, int count) {
		...
		if (!AMapUtil.IsEmptyOrNullString(newText)) {
			...
			Inputtips inputTips = new Inputtips(GaoDeSearchComInfo.this, inputquery);
